...
import {
  MyHead,
  MySection, MySocialLink, NewLabel
} from '../components'

const backgroundIcons = [
  'fab fa-js', 'fab fa-css3', 'fab fa-html5', 'fas fa-code', 'fas fa-code-branch', 'fas fa-file-code',
  'fas fa-terminal', 'fab fa-node', 'fab fa-aws', 'fas fa-project-diagram']
const IconColumn = (props) => <div className='d-flex flex-column text-center' {...props} >
  {new Array(20).fill(1).map((_, i) => <i className={`${
    backgroundIcons[i % backgroundIcons.length]
    // backgroundIcons[Math.random() * backgroundIcons.length | 0]
  } fa-4x text-light mb-5`} />)}
</div>
      ...
      <div className='row align-items-center h-100'>
        <div className='offset-md-8 col-md-4 offset-lg-6 col-lg-6 d-none d-md-flex flex-column h-100 position-relative'>
          <img src='/static/images/half-size-herman.png'
            ...
            style={{
              // 'WebkitMaskImage': '-webkit-gradient(linear, center top, center bottom, color-stop(0.75, rgba(255,255,255,1)), color-stop(1.00, rgba(255,255,255,1)))',
